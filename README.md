...
`upstream-base` tags where the fork started. Upstream has been merged since, so the fork delta is
the diff against the last upstream commit merged:

```
git diff $(git merge-base main upstream/master) main -- common/src/main/java
```
