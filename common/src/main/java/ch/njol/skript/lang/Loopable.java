...
public interface Loopable<T> {
	...
	/**
	 * Returns the same as {@link Expression#getArray(PlatformEvent)} but as an iterator. This method should be overridden by expressions intended to be looped to increase performance.
	 * @see SimpleExpression#iterator(PlatformEvent)
	 *
	 ...
	 */
	@Nullable Iterator<? extends T> iterator(PlatformEvent event);
