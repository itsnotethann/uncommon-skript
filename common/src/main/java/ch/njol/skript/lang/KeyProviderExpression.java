 ...
 * <h2>Contract</h2>
 * <ul>
 *     <li>Neither {@link #getArrayKeys(PlatformEvent)} nor {@link #getAllKeys(PlatformEvent)} should ever be called without
 *     a corresponding {@link #getArray(PlatformEvent)} or {@link #getAll(PlatformEvent)} call.</li>
 *     <li>{@link #getArrayKeys(PlatformEvent)} and {@link #getAllKeys(PlatformEvent)} should only be called iff {@link #canReturnKeys()}
 *     returns {@code true}.</li>
 *     <li>A caller may ask only for values and does not have to invoke either {@link #getArrayKeys(PlatformEvent)} or
 *     {@link #getAllKeys(PlatformEvent)}.</li>
 *     <li>{@link #getArrayKeys(PlatformEvent)} might be called after the corresponding {@link #getArray(PlatformEvent)}</li>
 *     <li>{@link #getAllKeys(PlatformEvent)} might be called after the corresponding {@link #getAll(PlatformEvent)}</li>
 ...
 * {@link #getArray(PlatformEvent)}
 * and return the keys during {@link #getArrayKeys(PlatformEvent)} (provided the cache is weak, safe and event-linked).
 ...
 * <h3>Caveats</h3>
 * <ol>
 *     <li>The caller may <i>never</i> ask for {@link #getArrayKeys(PlatformEvent)}.
 ...
 *     .</li>
 * </ol>
 * Note that the caller may <i>never</i> ask for {@link #getArrayKeys(PlatformEvent)} and so the cache should be disposed of
 * in a timely manner.
 * <br/>
 * <pre>{@code
...
public interface KeyProviderExpression<T> extends Expression<T>, KeyedIterableExpression<T> {
	 ...
	 * {@link #getArray(PlatformEvent)} values array.
	 * <br/>
	 * This should <b>only</b> be called immediately after a {@link #getArray(PlatformEvent)} invocation,
	 ...
	 * @return A set of keys, of the same length as {@link #getArray(PlatformEvent)}
	 * @throws IllegalStateException If this was not called directly after a {@link #getArray(PlatformEvent)} call
	 * or if {@link #canReturnKeys()} returns {@code false}
	 */
	@NotNull String @NotNull [] getArrayKeys(PlatformEvent event) throws IllegalStateException;

	/**
	 * A set of keys, matching the length and order of the immediately-previous
	 * {@link #getAll(PlatformEvent)} values array.
	 * <br/>
	 * This should <b>only</b> be called immediately after a {@link #getAll(PlatformEvent)} invocation,
	 * and iff {@link #canReturnKeys()} returns {@code true}.
	 * If it is called without a matching values request (or after a delay) then the behaviour
	 * is undefined, in which case:
	 * <ul>
	 ...
	 *
	 * @param event The event context
	 * @return A set of keys, of the same length as {@link #getAll(PlatformEvent)}
	 * @throws IllegalStateException If this was not called directly after a {@link #getAll(PlatformEvent)} call
	 * or if {@link #canReturnKeys()} returns {@code false}
	 */
	default @NotNull String @NotNull [] getAllKeys(PlatformEvent event) {
	 ...
	 */
	@Override
	default Iterator<KeyedValue<T>> keyedIterator(PlatformEvent event) {
	 ...
	 * <br/>
	 * If this returns false, then {@link #getArrayKeys(PlatformEvent)} and {@link #getAllKeys(PlatformEvent)} should never be called.
	 *
